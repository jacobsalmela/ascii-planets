...
The \planet\ is under constant threat from calamities and \pirate\ raids.  \disasters\ are checked after the \diplomacy\ phase if any dice showing 1 still remain.  These dice are re-rolled and their new value will determine what disaster strikes.
\begin{enumerate}
...
\begin{itemize}
  \item \pandemic\ only affects players who have not yet researched \cure\ (see \nameref{sec:disasters}, p.\pageref{sec:disasters}). The affected player must cross over two \astronauts\ on the \population\ track or gain one \unhappiness.
